...
def pattern_to_phrase(pattern: Any) -> str:
...
def load_taxonomy(path: str):
    ...
    for alias, canonical in data.get("aliases", {}).items():
        ...
        if alias_n and canonical_n:
            term_to_canonical.setdefault(alias_n, canonical_n)
    ...
    for entry in data.get("skills", []):
        ...
        all_terms.add(skill)
        term_to_canonical.setdefault(skill, skill)

        for alias in entry.get("aliases", []):
            alias_n = normalize_text(alias)
            all_terms.add(alias_n)
            term_to_canonical.setdefault(alias_n, skill)

        pattern_phrase = pattern_to_phrase(entry.get("pattern"))
        if pattern_phrase:
            all_terms.add(pattern_phrase)
            term_to_canonical.setdefault(pattern_phrase, skill)
...
if __name__ == "__main__":
    ...
    parser.add_argument("taxonomy_file", nargs="?", default=None)
    parser.add_argument("output_file", nargs="?", default=None)
    parser.add_argument("--input", dest="input_opt", default=None)
    parser.add_argument("--taxonomy", dest="taxonomy_opt", default=None)
    parser.add_argument("--output", default=None)

    args = parser.parse_args()

    input_path = args.input_opt or args.input_file
    taxonomy_path = args.taxonomy_opt or args.taxonomy_file
    output_path = args.output or args.output_file

    if not input_path or not taxonomy_path:
        parser.error("Input and taxonomy are required. Provide positional args or --input/--taxonomy.")

    text = Path(input_path).read_text(encoding="utf-8")

    extractor = LayerAExtractor(taxonomy_path)
    ...
    if output_path:
        ...
        output_target.write_text(output_json, encoding="utf-8")
