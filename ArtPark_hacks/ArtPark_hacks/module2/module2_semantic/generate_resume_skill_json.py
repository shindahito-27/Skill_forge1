...
DEFAULT_RESUME_PDF = REPO_ROOT / "main_Resume-2.pdf"


def _select_default_taxonomy_path() -> Path:
    candidates = [
        SEMANTIC_DIR / "skill_taxonomy_500plus(2).json",
        MODULE2_ROOT / "skill_taxonomy_500plus(1).json",
...
DEFAULT_TAXONOMY_PATH = _select_default_taxonomy_path()
DEFAULT_OUTPUT_PATH = Path("output/resume/module_2/B/layer_a_semantic_resume.json")

# Ensure shared taxonomy loaders resolve to an existing taxonomy file.
os.environ.setdefault("SKILL_TAXONOMY_PATH", str(DEFAULT_TAXONOMY_PATH))
...
def _load_skill_metadata(taxonomy_path: Path) -> Dict[str, Dict[str, str]]:
    ...
    for entry in payload.get("skills", []):
        ...
        sub_category_label = str(entry.get("sub_category") or entry.get("subcategory") or "").strip()
        ...
        meta[canonical] = {
            ...
            "sub_category": sub_category_label or "General",
        }
    return meta


def _pattern_to_phrase(pattern: Any) -> str:
    ...
    for token in pattern:
        ...
        if not isinstance(lower, str):
            return ""
        normalized = normalize_skill_name(lower)
        if not normalized:
            return ""
        tokens.append(normalized)

    return normalize_skill_name(" ".join(tokens))


def _build_variant_map(taxonomy_path: Path) -> Dict[str, Set[str]]:
    ...
    for entry in payload.get("skills", []):
        ...
        if pattern_phrase:
            variants[canonical].add(pattern_phrase)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional positional plain text input (pipeline mode).",
    )
    parser.add_argument(
        "taxonomy_pos",
        nargs="?",
        type=Path,
        default=None,
        help="Optional positional taxonomy JSON path (pipeline mode).",
    )
    parser.add_argument(
        "output_pos",
        nargs="?",
        type=Path,
        default=None,
        help="Optional positional output JSON path (pipeline mode).",
    )
    parser.add_argument(
        "--resume-pdf",
        type=Path,
...
def main() -> None:
    ...
    if args.text_file is None and args.text_file_pos is not None:
        args.text_file = args.text_file_pos
    if args.taxonomy is None:
        args.taxonomy = args.taxonomy_pos or DEFAULT_TAXONOMY_PATH
    if args.output is None:
        args.output = args.output_pos or DEFAULT_OUTPUT_PATH
    ...
    variants = _build_variant_map(args.taxonomy)
    ...
    for skill in all_skills:
        ...
        meta = skill_meta.get(
            canonical,
            {"category": "hard_skill", "taxonomy_category": "uncategorized", "sub_category": "General"},
        )
